    ...
    <div class="dash-content">
		...
		<div class="row no-margin">
		    <div class="col-md-12">
		        ...
		        <!--<img src="asset/img/location_waves.gif" alt="img" style="width: 25%;margin-left:12%;" />-->
		    </div>
		</div>
		...
		<div class="row no-margin">
		        ...
		        <div class="col-md-6">
		            <dl class="dl-horizontal left-right">
		                ...
		                <dd id="request_id">{{$request->id}}</dd>
...
@section('scripts')
    ...
    <script type="text/jsx">
			...
			render: function(){
				return (
					<form action="{{url('cancel/ride')}}" method="POST">
						...
						<input type="hidden" name="card_id" value={this.props.checkState.card_id} />
			            <div className="status">
			                <h6>@lang('user.status')</h6>
			                <p>@lang('user.ride.finding_driver')</p>
			            </div>

		            	<button type="submit" className="full-primary-btn btn-danger fare-btn">@lang('user.ride.cancel_request')</button> 
		            </form>
				);
			}
		});
			...
			render: function(){
				return (
					...
					<input type="hidden" name="card_id" value={this.props.checkState.card_id} />
			            ...
			            <CancelReason/>
		            	<button type="button" className="full-primary-btn btn-danger" data-toggle="modal" data-target="#cancel-reason">@lang('user.ride.cancel_request')</button>
		            	<br/>
			...
			render: function(){
				...
				<div>
					...
					<div className="status">
                    	...
                    	<button type="submit" className="full-primary-btn fare-btn">PAY WITH PAYPAL</button>   
                    </form>
		        </div>
				);
			}
		...
		var DroppedAndRazorpay = React.createClass({
			...
			chk:function(){
			var request_id =  this.props.checkState.id;
            var amounts = this.props.checkState.payment.total;
            var amount = Math.round(amounts);
		     ...
		     var options = {
		        ...
		        handler: function (response){
	                ...
	                $.ajax({
	                   ...
	                   data: {
	                    razorpay_payment_id: response.razorpay_payment_id,request_id: request_id,amount:amount
	                   }, 
			...
			render: function(){
				return (
				<div>
					<form id="rzp-footer-form" method="POST" action="{{url('/payment')}}">
						{{ csrf_field() }}</input>
					<div className="status">
		                <h6>@lang('user.status')</h6>
		                <p>@lang('user.ride.dropped_ride')</p>
                        ...
                        <hr />
		            	<h5><strong>@lang('user.ride.invoice')</strong></h5>
		            	<input type="hidden" id="request_id" name="request_id" value={this.props.checkState.id} />
		            	<input type="hidden" name="card_id" value={this.props.checkState.card_id} />
		            	<dl className="dl-horizontal left-right">
                            <dt>@lang('user.ride.base_price')</dt>
                            <dd>{this.props.currency}{this.props.checkState.payment.fixed}</dd>
                    	...
                    	<button className="full-primary-btn fare-btn" id="paybtn" type="button"  onClick={this.chk}>Pay with Razorpay</button>
                    </form>
		        </div>
				);
			}
			...
			render: function(){
				...
				{{ csrf_field() }}</input>
                    <div className="rate-review">
                        ...
                        <div className="rating-outer">
                            <input type="hidden" value="1" name="rating" className="rating" placeholder="123"/>
                        </div>
						<input type="hidden" name="request_id" value={this.props.checkState.id} />
						<input type="hidden" name="card_id" value={this.props.checkState.card_id} />
