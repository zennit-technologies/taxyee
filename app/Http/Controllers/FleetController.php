    ...
    public function dashboard()
    {
        try{
            ...
            $getting_ride = UserRequests::has('user')
                    ...
                    ->orderBy('id','desc');
            $ride = $getting_ride->get();

            $rides = UserRequests::has('user')->whereHas('provider', function($query) {
                            $query->where('fleet', Auth::user()->id );
                        })->orderBy('id','desc')->get();
            $all_rides = $getting_ride->get()->pluck('id');
            
            $cancel_rides = UserRequests::has('user')
                    ->whereHas('provider', function($query) {
                            $query->where('fleet', Auth::user()->id );
            ...
            $service = ServiceType::count();
              ...
              $revenue = UserRequests::join('user_request_payments', 'user_requests.id', '=', 'user_request_payments.request_id')->whereHas('provider', function($query) {
                            $query->where('fleet', Auth::user()->id );
                        })->sum('user_request_payments.total');
            
            $providers = Provider::where('fleet', Auth::user()->id)->take(10)->orderBy('rating','desc')->get();

            return view('fleet.dashboard',compact('providers','all_rides','service','rides','cancel_rides','revenue','user_cancelled','provider_cancelled'));
        }
        catch(Exception $e){
            dd($e->getMessage());
